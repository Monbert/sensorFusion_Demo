    ...
    for (int j = 0; j < a->column; j++){
        ...
        for (int k = 0; k < j; k++){
            ...
            for(int i1 = 0; i1 < col_a.row; i1++){
                r->data[k * r->column + j] += col_a.data[i1] * q->data[i1 * q->column + k];
            }
            for (int i2 = 0; i2 < a->column; i2++){
                col_q.data[i2] -= r->data[k * r->column + j] * q->data[i2 * q->column + k];
            }
        }
        double temp = matrix_norm2(&col_q);
...
void calculate_eigenvector(matrix *a, matrix *eigen_vector, matrix *eigen_values){
    ...
    for (int count = 0; count < num; count++){
        ...
        double sum1 = eigen_vector->data[(eigen_vector->row - 1) * eigen_vector->column + count] = 1;
        for (int i2 = temp.row - 2; i2 >= 0; i2--){
            double sum2 = 0;
            for (int j2 = i2 + 1; j2 < temp.column; j2++){
                sum2 += temp.data[i2 * temp.column + j2] * eigen_vector->data[j2 * eigen_vector->column + count];
            }
